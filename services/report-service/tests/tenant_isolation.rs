...
fn test_database_url() -> String {
    std::env::var("REPORT_TEST_DATABASE_URL").unwrap_or_else(|_| {
        let base = admin_url();
        match base.rsplit_once('/') {
            Some((prefix, _)) => format!("{prefix}/openfoundry_report_test"),
            None => base,
        }
    })
}
...
async fn pool() -> PgPool {
    ...
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = 'openfoundry_report_test')",
    )
    .fetch_one(&admin)
    .await
    .expect("exists");
    if !exists {
        sqlx::query("CREATE DATABASE openfoundry_report_test")
            .execute(&admin)
            .await
            .expect("create db");
    }
    ...
    .await
    .ok();
    sqlx::query("GRANT ALL PRIVILEGES ON DATABASE openfoundry_report_test TO openfoundry_app")
        .execute(&admin)
        .await
        .ok();
...
#[tokio::test]
async fn tenant_b_cannot_see_tenant_a_report_definition() {
    let pool = pool().await;
    ...
    sqlx::query(
        r#"INSERT INTO report_definitions (id, name, description, owner, generator_kind, dataset_name, tenant_id)
           VALUES ($1, $2, '', 'owner', 'pdf', 'sales_fact_daily', $3)"#,
    )
    .bind(report_id)
    ...
    let visible: Option<Uuid> =
        sqlx::query_scalar("SELECT id FROM report_definitions WHERE id = $1")
            .bind(report_id)
