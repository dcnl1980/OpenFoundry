...
fn test_database_url() -> String {
    std::env::var("NOTIFICATION_TEST_DATABASE_URL").unwrap_or_else(|_| {
        let base = admin_url();
        match base.rsplit_once('/') {
            Some((prefix, _)) => format!("{prefix}/openfoundry_notification_test"),
            None => base,
        }
    })
}
...
async fn pool() -> PgPool {
    ...
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = 'openfoundry_notification_test')",
    )
    .fetch_one(&admin)
    .await
    .expect("exists");
    if !exists {
        sqlx::query("CREATE DATABASE openfoundry_notification_test")
            .execute(&admin)
            .await
            .expect("create db");
    }
    ...
    .await
    .ok();
    sqlx::query("GRANT ALL PRIVILEGES ON DATABASE openfoundry_notification_test TO openfoundry_app")
        .execute(&admin)
        .await
        .ok();
...
#[tokio::test]
async fn tenant_b_cannot_see_tenant_a_notification() {
    let pool = pool().await;
    ...
    sqlx::query(
        r#"INSERT INTO notifications (id, user_id, title, body, tenant_id)
           VALUES ($1, $2, 'hello', 'world', $2)"#,
    )
    ...
    let visible: Option<Uuid> = sqlx::query_scalar("SELECT id FROM notifications WHERE id = $1")
        .bind(notification_id)
