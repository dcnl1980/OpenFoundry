...
fn test_database_url() -> String {
    std::env::var("FUSION_TEST_DATABASE_URL").unwrap_or_else(|_| {
        let base = admin_url();
        match base.rsplit_once('/') {
            Some((prefix, _)) => format!("{prefix}/openfoundry_fusion_test"),
            None => base,
        }
    })
}
...
async fn pool() -> PgPool {
    ...
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = 'openfoundry_fusion_test')",
    )
    .fetch_one(&admin)
    .await
    .expect("exists");
    if !exists {
        sqlx::query("CREATE DATABASE openfoundry_fusion_test")
            .execute(&admin)
            .await
            .expect("create db");
    }
    ...
    .await
    .ok();
    sqlx::query("GRANT ALL PRIVILEGES ON DATABASE openfoundry_fusion_test TO openfoundry_app")
        .execute(&admin)
        .await
        .ok();
...
#[tokio::test]
async fn tenant_b_cannot_see_or_delete_tenant_a_match_rule() {
    let pool = pool().await;
    ...
    sqlx::query(
        r#"INSERT INTO fusion_match_rules (id, name, tenant_id)
           VALUES ($1, $2, $3)"#,
    )
    .bind(rule_id)
    ...
    let visible: Option<Uuid> =
        sqlx::query_scalar("SELECT id FROM fusion_match_rules WHERE id = $1")
            .bind(rule_id)
            .fetch_optional(&mut *tx_b)
            .await
            .expect("select");
    let deleted = sqlx::query("DELETE FROM fusion_match_rules WHERE id = $1")
        .bind(rule_id)
        .execute(&mut *tx_b)
        .await
        .expect("delete")
