...
fn test_database_url() -> String {
    std::env::var("PIPELINE_TEST_DATABASE_URL").unwrap_or_else(|_| {
        let base = admin_url();
        match base.rsplit_once('/') {
            Some((prefix, _)) => format!("{prefix}/openfoundry_pipeline_test"),
            None => base,
        }
    })
}
...
async fn pool() -> PgPool {
    ...
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = 'openfoundry_pipeline_test')",
    )
    .fetch_one(&admin)
    .await
    .expect("exists");
    if !exists {
        sqlx::query("CREATE DATABASE openfoundry_pipeline_test")
            .execute(&admin)
            .await
            .expect("create db");
    }
    ...
    .await
    .ok();
    sqlx::query("GRANT ALL PRIVILEGES ON DATABASE openfoundry_pipeline_test TO openfoundry_app")
        .execute(&admin)
        .await
        .ok();
...
#[tokio::test]
async fn tenant_b_cannot_see_or_mutate_tenant_a_pipeline() {
    let pool = pool().await;
    ...
    sqlx::query(
        r#"INSERT INTO pipelines (id, name, description, owner_id, tenant_id)
           VALUES ($1, $2, '', $3, $3)"#,
    )
    .bind(pipeline_id)
    .bind(format!("p-{}", pipeline_id))
    .bind(tenant_a)
    .execute(&mut *tx_a)
    .await
    .expect("insert");
    ...
    let visible: Option<Uuid> = sqlx::query_scalar("SELECT id FROM pipelines WHERE id = $1")
        .bind(pipeline_id)
        .fetch_optional(&mut *tx_b)
        .await
        .expect("select");
    let updated = sqlx::query("UPDATE pipelines SET name = 'hacked' WHERE id = $1")
        .bind(pipeline_id)
        .execute(&mut *tx_b)
        .await
        .expect("update")
