...
fn test_database_url() -> String {
    std::env::var("ML_TEST_DATABASE_URL").unwrap_or_else(|_| {
        let base = admin_url();
        match base.rsplit_once('/') {
            Some((prefix, _)) => format!("{prefix}/openfoundry_ml_test"),
            None => base,
        }
    })
}
...
async fn pool() -> PgPool {
    ...
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = 'openfoundry_ml_test')",
    )
    .fetch_one(&admin)
    .await
    .expect("exists");
    if !exists {
        sqlx::query("CREATE DATABASE openfoundry_ml_test")
            .execute(&admin)
            .await
            .expect("create db");
    }
    ...
    .await
    .ok();
    sqlx::query("GRANT ALL PRIVILEGES ON DATABASE openfoundry_ml_test TO openfoundry_app")
        .execute(&admin)
        .await
        .ok();
...
#[tokio::test]
async fn tenant_b_cannot_see_or_delete_tenant_a_experiment() {
    let pool = pool().await;
    ...
    sqlx::query(
        r#"INSERT INTO ml_experiments (id, name, tenant_id)
           VALUES ($1, $2, $3)"#,
    )
    .bind(experiment_id)
    .bind(format!("exp-{}", experiment_id))
    .bind(tenant_a)
    .execute(&mut *tx_a)
    .await
    .expect("insert");
    ...
    let visible: Option<Uuid> = sqlx::query_scalar("SELECT id FROM ml_experiments WHERE id = $1")
        .bind(experiment_id)
        .fetch_optional(&mut *tx_b)
        .await
        .expect("select");
    let deleted = sqlx::query("DELETE FROM ml_experiments WHERE id = $1")
        .bind(experiment_id)
        .execute(&mut *tx_b)
        .await
        .expect("delete")
